...
class algorithm {
    ...
    for (int j = 0; j < n; ++j) {
      for (int i = 0; i < n; ++i) {
        ...
        if (std::abs(C[j * n + i] - A[j * n + i]) > epsilon) {
          printf(
              "ERROR: %s did not validate at C(%d,%d) = %lf != A(%d,%d) = "
              "%lf\n",
              name.c_str(), i, j, C[j * n + i], i, j, A[j * n + i]);
          printf("ERROR: %g; %g < %g < %g\n", epsilon, A[j * n + i] - epsilon,
                 C[j * n + i], A[j * n + i] + epsilon);
          return false;
        }
      }
    }
 ...
 public:
  algorithm(const std::string& alg_name, bool t)
      : name(alg_name), is_tiled(t) {}
  ...
  double operator()(double* A, int n, int b, int trials) {
    ...
    printf("%s %d %d %d %d %lf %lf\n", name.c_str(), g_num_tbb_threads, trials,
           n, b, elapsed_time, elapsed_time / trials);
    free(A0);
    free(C);
    return elapsed_time;
  }
...
static void call_dsyr2k(double*** tile, int b, int k, int j, int i) {
  ...
  if (i == j) {  // Diagonal block
    ...
    dgemm(&transa, &transb, &b, &b, &b, &alpha, L1_block, &b, L2_block, &b,
          &beta, A_block, &b);
  }
  return;
}
...
class algorithm_depend : public algorithm {
 ...
 protected:
  virtual void func(void* ptr, int n, int b) {
    ...
    tbb::flow::graph g;
    for (int k = p - 1; k >= 0; --k) {
      c[k] = new continue_type(
          g, [=](const tbb::flow::continue_msg&) { call_dpotf2(tile, b, k); });
      t[k] = new continue_ptr_type[p];
      u[k] = new continue_ptr_type*[p];

      for (int j = k + 1; j < p; ++j) {
        t[k][j] = new continue_type(g, [=](const tbb::flow::continue_msg&) {
          call_dtrsm(tile, b, k, j);
        });
        make_edge(*c[k], *t[k][j]);
        ...
        for (int i = k + 1; i <= j; ++i) {
          u[k][j][i] =
              new continue_type(g, [=](const tbb::flow::continue_msg&) {
                call_dsyr2k(tile, b, k, j, i);
              });
...
bool process_args(int argc, char* argv[]) {
  utility::parse_cli_arguments(
      ...
      utility::cli_argument_pack()
          ...
          .positional_arg(g_n, "size",
                          "the row/column size of NxN matrix (size <= 46000)")
          .positional_arg(
              g_b, "blocksize",
              "the block size; size must be a multiple of the blocksize")
          .positional_arg(g_num_trials, "num_trials",
          ...
          .positional_arg(
              ...
              "                     output_prefix_X.txt; where X is the "
              "algorithm used\n"
              "                 if output_prefix is not provided, no output "
              "will be written")
          .positional_arg(
              g_alg_name, "algorithm",
              "name of the used algorithm - can be dpotrf or depend")
          .positional_arg(g_num_tbb_threads, "num_tbb_threads",
                          "number of started TBB threads")

          .arg(g_input_file_name, "input_file",
  ...
  if (g_n / g_b > 256) {
    ...
    printf(
        "ERROR: size / blocksize must be less or equal 256, but %d / %d = %d\n",
        g_n, g_b, g_n / g_b);
    return false;
  }
...
int main(int argc, char* argv[]) {
  ...
  // Init algorithms
  algmap.insert(
      std::pair<std::string, algorithm*>("dpotrf", new algorithm_dpotrf));
  algmap.insert(
      std::pair<std::string, algorithm*>("depend", new algorithm_depend));
